...
for SCHEDULER in "${SCHEDULERS[@]}"; do
    for i in "${!ENERGY_LEVELS[@]}"; do
        ...
        echo "运行测试（时长: ${SIMULATION_TIME}ms）..."
        "$BINARY" "$CONFIG_FILE" "$TASKS_FILE" $SIMULATION_TIME -t "$TRACE_FILE" > "$OUTPUT_LOG" 2>&1
