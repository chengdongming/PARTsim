...
With the checked-in `resume: false`, any current-request terminal collision fails closed. A formal interrupted run is resumed by repeating its original execute command with `--resume`, for example `python3 scripts/run_v9_3_b4_performance.py --config <stage-config> --execute --resume`; tracked configuration must not be edited. The CLI override is execution-only and is rejected with plan, freeze, or analysis modes. Resume skips only readable terminal results whose semantic request ID, execution identity, and terminal marker match the current request. Corrupt, partial, non-terminal, or mismatched evidence fails closed. On a worker failure, no new requests are submitted; queued work is cancelled where possible, running work is awaited, and already atomically written terminals remain valid for a later resume.
