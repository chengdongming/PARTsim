...
import argparse, hashlib, json, re, shutil, subprocess, sys, tempfile, zipfile, yaml
sys.dont_write_bytecode=True
from ASAP_BLOCK_validation_common_v1_3_12 import *
VERSION='1.3.12'
ALGORITHM='SHA256_DOMAIN_SEPARATED_CANONICAL_V1_3_12'
...
'schema':'ASAP_BLOCK_experiment_schema_v1_3_12.yaml','dictionary':'ASAP_BLOCK_data_dictionary_v1_3_12.yaml',
'canonical':'ASAP_BLOCK_canonical_serialization_v1_3_12.yaml','interface_manifest':'ASAP_BLOCK_machine_interface_manifest_v1_3_12.yaml',
'formal':'ASAP_BLOCK_formal_contract_template_v1_3_12.yaml','generator':'ASAP_BLOCK_generator_contract_template_v1_3_12.yaml',
'simulation':'ASAP_BLOCK_simulation_contract_template_v1_3_12.yaml','trace':'ASAP_BLOCK_trace_generator_contract_template_v1_3_12.yaml',
'acceptance':'ASAP_BLOCK_acceptance_report_template_v1_3_12.yaml','common':'ASAP_BLOCK_validation_common_v1_3_12.py',
'artifact_validator':'ASAP_BLOCK_artifact_validator_v1_3_12.py','result_validator':'ASAP_BLOCK_result_validator_v1_3_12.py',
'acceptance_validator':'ASAP_BLOCK_acceptance_report_validator_v1_3_12.py','plan_template':'ASAP_BLOCK_run_plan_definition_template_v1_3_12.csv',
'plan_dependencies_template':'ASAP_BLOCK_run_plan_dependencies_template_v1_3_12.csv','execution_log_template':'ASAP_BLOCK_run_execution_log_template_v1_3_12.csv',
'per_task_results_template':'ASAP_BLOCK_per_task_results_template_v1_3_12.csv',
'validation_summary':'ASAP_BLOCK_v1_3_12_验证摘要.txt'}
...
def validate(root:Path,check_report=True,run_subvalidator_checks=True):
    ...
    expected_schema_refs={
        ...
        'per_task_results_template_file':FILES['per_task_results_template'],
    }
    ...
    result_source=read_text_strict(p['result_validator'])
    if "LOC_THETA_CW_SOURCE_CERTIFICATION='CERTIFIED_TASKSET'" not in result_source:
        v.err('LOC-Theta^cw source certification predicate must require CERTIFIED_TASKSET')
    expected_failure_codes=['NONE','NO_CANDIDATE','SOLVER_TIMEOUT','NUMERIC_ERROR','UPSTREAM_PREFIX_FAILURE','DEPENDENCY_NOT_APPLICABLE','DOMINANCE_INVARIANT_VIOLATION','UNKNOWN_CORE_STATUS','INTERNAL_CONFORMANCE_FAILURE']
    if s.get('enums',{}).get('task_failure_reason_code')!=expected_failure_codes:v.err('task failure code enum mismatch')
    task_schema=st.get('per_task_results.csv',{});task_fields=dt.get('per_task_results.csv',{}).get('fields',{})
    code_spec=task_fields.get('task_failure_reason_code',{});detail_spec=task_fields.get('task_failure_detail',{})
    if (code_spec.get('type'),code_spec.get('nullable'),code_spec.get('enum_ref'),code_spec.get('field_class'))!=('enum',False,'task_failure_reason_code','required'):v.err('task failure code field contract mismatch')
    if (detail_spec.get('type'),detail_spec.get('nullable'),detail_spec.get('format'),detail_spec.get('field_class'),detail_spec.get('max_length'))!=('string',True,'canonical_task_failure_detail_v1_3_12','conditionally_required','256'):v.err('task failure detail field contract mismatch')
    task_order=task_schema.get('canonical_column_order',[])
    if len(task_order)!=41 or task_order[14]!='task_failure_reason_code' or task_order[25]!='task_failure_detail':v.err('task failure canonical column positions mismatch')
    pre_task=c.get('preimages',{}).get('task_result_hash',{})
    if 'task_failure_reason_code' not in str(pre_task.get('include')) or 'task_failure_detail' not in str(pre_task.get('include')) or pre_task.get('raw_failure_reason')!='forbidden':v.err('task-result preimage does not bind structured failure provenance')
    expected_detail={'NONE':None,'NO_CANDIDATE':'closure exhausted through task deadline','SOLVER_TIMEOUT':None,'NUMERIC_ERROR':'numeric guard rejected analysis','UPSTREAM_PREFIX_FAILURE':None,'DEPENDENCY_NOT_APPLICABLE':None,'DOMINANCE_INVARIANT_VIOLATION':'local result violated frozen carry-in dominance','UNKNOWN_CORE_STATUS':'unrecognized core solver status','INTERNAL_CONFORMANCE_FAILURE':'internal analyzer conformance failure'}
    provenance=f.get('analysis_contract',{}).get('task_failure_provenance',{})
    if provenance.get('exact_detail_by_code')!=expected_detail:v.err('formal task failure detail matrix mismatch')
    if set(provenance.get('status_code_matrix',{}))!=set(s.get('enums',{}).get('task_solver_status',[])):v.err('formal task failure status matrix coverage mismatch')
    if provenance.get('unknown_raw_or_origin_policy')!='FAIL_CLOSED' or provenance.get('raw_failure_reason_is_formal') is not False:v.err('formal raw failure normalization policy mismatch')
    if 'def normalize_task_failure_reason(' not in result_source or 'return code,raw_failure_reason' in result_source:v.err('result validator raw failure normalization implementation missing or unsafe')
    ...
    req=set(f.get('output_contract',{}).get('required_files',[]));er={'config.yaml','generator_contract.yaml','simulation_contract.yaml','trace_generator_contract.yaml','formal_contract.yaml','acceptance_report.yaml','manifest.json','git_commit.txt','git_status.txt','sha256sum.txt','ASAP_BLOCK_validation_common_v1_3_12.py','ASAP_BLOCK_artifact_validator_v1_3_12.py','ASAP_BLOCK_result_validator_v1_3_12.py','ASAP_BLOCK_acceptance_report_validator_v1_3_12.py','ASAP_BLOCK_formal_contract_template_v1_3_12.yaml'}|set(st)|{rec.get('file') for rec in f.get('artifact_bindings',{}).values() if isinstance(rec,dict)}
    if not er<=req:v.err(f'formal output contract missing:{sorted(er-req)}')
    # CSV templates are exact schema headers.
    for key,tn in [('plan_template','run_plan_definition.csv'),('plan_dependencies_template','run_plan_dependencies.csv'),('execution_log_template','run_execution_log.csv'),('per_task_results_template','per_task_results.csv')]:
...
def refresh_derived(root):
    """Explicit release-builder step for the acyclic manifest/binding/sidecar DAG."""
    schema=load_yaml_strict(root/FILES['schema'])
    manifest={'machine_interface_manifest_metadata':{'name':'ASAP_BLOCK_machine_interface_manifest','version':VERSION,'authority':'normative_machine_interface_projection'},'projection':projection(schema)}
    mp=root/FILES['interface_manifest'];mp.write_text(yaml.safe_dump(manifest,sort_keys=False,allow_unicode=True),encoding='utf-8')
    md=root/FILES['markdown'];txt=read_text_strict(md)
    txt,n=re.subn(r'(ASAP_BLOCK_MACHINE_INTERFACE_MANIFEST_SHA256: )[0-9a-f]{64}',r'\g<1>'+sha256_file(mp),txt,count=1)
    if n!=1:raise ValueError('Markdown manifest hash marker is not unique')
    md.write_text(txt,encoding='utf-8')
    fp=root/FILES['formal'];formal=load_yaml_strict(fp)
    for key,rec in formal.get('artifact_bindings',{}).items():
        if key not in FILES:raise ValueError(f'unknown artifact binding:{key}')
        rec['file']=FILES[key];rec['sha256']=sha256_file(root/FILES[key])
    fp.write_text(yaml.safe_dump(formal,sort_keys=False,allow_unicode=True),encoding='utf-8')
    for fn in FILES.values():write_sidecar(root/fn)
    if (root/REPORT).exists():write_sidecar(root/REPORT)

def build_release_zip(root,zip_path):
    """Build and immediately verify a byte-deterministic regular-file ZIP."""
    zip_path=zip_path.resolve();files=sorted((x for x in root.iterdir() if x.is_file() and not x.is_symlink()),key=lambda x:x.name.encode('utf-8'))
    root_name=root.name;tmp=zip_path.with_suffix(zip_path.suffix+'.tmp')
    fixed=(1980,1,1,0,0,0)
    with zipfile.ZipFile(tmp,'w',compression=zipfile.ZIP_DEFLATED,compresslevel=9) as z:
        info=zipfile.ZipInfo(root_name+'/',fixed);info.create_system=3;info.external_attr=(0o40755<<16)|0x10;z.writestr(info,b'')
        for path in files:
            info=zipfile.ZipInfo(root_name+'/'+path.name,fixed);info.create_system=3;info.external_attr=0o100644<<16;info.compress_type=zipfile.ZIP_DEFLATED
            z.writestr(info,path.read_bytes(),compresslevel=9)
    tmp.replace(zip_path)
    expected={root_name+'/'+x.name:sha256_file(x) for x in files}
    with zipfile.ZipFile(zip_path,'r') as z:
        actual={i.filename for i in z.infolist() if not i.is_dir()}
        if actual!=set(expected):raise ValueError('release ZIP member set mismatch')
        for info in z.infolist():
            mode=(info.external_attr>>16)&0o170000
            if not info.is_dir() and mode not in {0,0o100000}:raise ValueError(f'non-regular ZIP member:{info.filename}')
            if not info.is_dir() and hashlib.sha256(z.read(info)).hexdigest()!=expected[info.filename]:raise ValueError(f'ZIP member hash mismatch:{info.filename}')
    manifest=Path(str(zip_path)+'.manifest')
    manifest.write_text(''.join(f'{expected[name]}  {name}\n' for name in sorted(expected,key=lambda x:x.encode('utf-8'))),encoding='utf-8')
    write_sidecar(zip_path);write_sidecar(manifest)
    return {'zip_file':zip_path.name,'zip_sha256':sha256_file(zip_path),'manifest_file':manifest.name,'manifest_sha256':sha256_file(manifest),'member_count':len(expected),'deterministic_timestamp':'1980-01-01T00:00:00Z'}
def self_test(root):
    ...
    def old_carry_in_source_enum(d):
        x=d/FILES['dictionary'];y=load_yaml_strict(x)
        y['tables']['per_task_results.csv']['fields']['carry_in_source_certification_status']['enum_ref']='task_certification_status'
        x.write_text(yaml.safe_dump(y,sort_keys=False,allow_unicode=True),encoding='utf-8');write_sidecar(x);bind(d,'dictionary')
    cases['old_carry_in_source_task_enum_rejected']=mutate(old_carry_in_source_enum)
    def missing_failure_code(d):
        x=d/FILES['schema'];y=load_yaml_strict(x);t=y['tables']['per_task_results.csv']
        y['enums'].pop('task_failure_reason_code');t['required'].remove('task_failure_reason_code');t['canonical_column_order'].remove('task_failure_reason_code')
        x.write_text(yaml.safe_dump(y,sort_keys=False,allow_unicode=True),encoding='utf-8');write_sidecar(x);bind(d,'schema')
    cases['v1_3_11_missing_failure_code_rejected']=mutate(missing_failure_code)
    def wrong_failure_enum_ref(d):
        x=d/FILES['dictionary'];y=load_yaml_strict(x);y['tables']['per_task_results.csv']['fields']['task_failure_reason_code']['enum_ref']='task_solver_status'
        x.write_text(yaml.safe_dump(y,sort_keys=False,allow_unicode=True),encoding='utf-8');write_sidecar(x);bind(d,'dictionary')
    cases['wrong_failure_code_enum_ref_rejected']=mutate(wrong_failure_enum_ref)
    def missing_failure_matrix(d):
        x=d/FILES['formal'];y=load_yaml_strict(x);y['analysis_contract']['task_failure_provenance'].pop('status_code_matrix')
        x.write_text(yaml.safe_dump(y,sort_keys=False,allow_unicode=True),encoding='utf-8');write_sidecar(x)
    cases['missing_failure_status_matrix_rejected']=mutate(missing_failure_matrix)
    def missing_hash_preimage_binding(d):
        x=d/FILES['canonical'];y=load_yaml_strict(x);y['preimages']['task_result_hash']['include']='all per_task_results row fields except task_result_hash'
        x.write_text(yaml.safe_dump(y,sort_keys=False,allow_unicode=True),encoding='utf-8');write_sidecar(x);bind(d,'canonical')
    cases['failure_fields_missing_from_hash_preimage_rejected']=mutate(missing_hash_preimage_binding)
    def raw_failure_copy(d):
        x=d/FILES['result_validator'];t=read_text_strict(x).replace('return code,TASK_FAILURE_DETAIL_BY_CODE[code]','return code,raw_failure_reason',1)
        x.write_text(t,encoding='utf-8');write_sidecar(x);bind(d,'result_validator')
    cases['raw_exception_copy_to_canonical_output_rejected']=mutate(raw_failure_copy)
    def downgraded_source_predicate(d):
        x=d/FILES['result_validator'];t=read_text_strict(x).replace(
            "LOC_THETA_CW_SOURCE_CERTIFICATION='CERTIFIED_TASKSET'",
            "LOC_THETA_CW_SOURCE_CERTIFICATION='CERTIFIED'",1)
        x.write_text(t,encoding='utf-8');write_sidecar(x);bind(d,'result_validator')
    cases['downgraded_source_certification_predicate_rejected']=mutate(downgraded_source_predicate)
    def fake_md(d):
    ...
    def bad_markdown_authority(d):
        x=d/FILES['markdown'];t=read_text_strict(x).replace('MACHINE_INTERFACE_AUTHORITY_V1_3_12','MACHINE_INTERFACE_AUTHORITY_V1_3_7',1)
    ...
    def old_per_task_header(d):
        x=d/FILES['per_task_results_template'];h,_=read_csv_strict(x);h=[q for q in h if q not in {'task_failure_reason_code','task_failure_detail'}]
        x.write_text(','.join(h)+'\n',encoding='utf-8');write_sidecar(x);bind(d,'per_task_results_template')
    cases['v1_3_11_39_column_task_header_rejected']=mutate(old_per_task_header)
    with tempfile.TemporaryDirectory() as q:
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('root',nargs='?',default='.');ap.add_argument('--write-report',action='store_true');ap.add_argument('--self-test',action='store_true');ap.add_argument('--refresh-derived',action='store_true');ap.add_argument('--build-zip');a=ap.parse_args();root=Path(a.root).resolve()
    if a.refresh_derived:refresh_derived(root)
    if a.write_report:
    ...
    if a.build_zip and o['status']=='PASSED':o['release_zip']=build_release_zip(root,Path(a.build_zip))
