...
from ASAP_BLOCK_validation_common_v1_3_11 import *
from ASAP_BLOCK_acceptance_report_validator_v1_3_11 import validate_report
VERSION='1.3.11'
SCHEMA='ASAP_BLOCK_experiment_schema_v1_3_11.yaml'
DICT='ASAP_BLOCK_data_dictionary_v1_3_11.yaml'
CANON='ASAP_BLOCK_canonical_serialization_v1_3_11.yaml'
COMMON='ASAP_BLOCK_validation_common_v1_3_11.py'
RESULT_VALIDATOR='ASAP_BLOCK_result_validator_v1_3_11.py'
ACCEPTANCE_VALIDATOR='ASAP_BLOCK_acceptance_report_validator_v1_3_11.py'
ARTIFACT_VALIDATOR='ASAP_BLOCK_artifact_validator_v1_3_11.py'
MARKDOWN='ASAP_BLOCK_实验配置与验收规范_v1_3_11_最终机器合同与验证闭合版.md'
INTERFACE_MANIFEST='ASAP_BLOCK_machine_interface_manifest_v1_3_11.yaml'
FORMAL_TEMPLATE='ASAP_BLOCK_formal_contract_template_v1_3_11.yaml'
GENERATOR_TEMPLATE='ASAP_BLOCK_generator_contract_template_v1_3_11.yaml'
SIMULATION_TEMPLATE='ASAP_BLOCK_simulation_contract_template_v1_3_11.yaml'
TRACE_TEMPLATE='ASAP_BLOCK_trace_generator_contract_template_v1_3_11.yaml'
ACCEPTANCE_TEMPLATE='ASAP_BLOCK_acceptance_report_template_v1_3_11.yaml'
PLAN_TEMPLATE='ASAP_BLOCK_run_plan_definition_template_v1_3_11.csv'
PLAN_DEPS_TEMPLATE='ASAP_BLOCK_run_plan_dependencies_template_v1_3_11.csv'
EXEC_LOG_TEMPLATE='ASAP_BLOCK_run_execution_log_template_v1_3_11.csv'
...
REQUEST_PAYLOAD_SCHEMA_VERSION='1.3.11'
...
def _validate_child_completeness(role,obj,v):
    ...
    if role=='generator':
        ...
        if p.get('parameter_cell_canonicalization_version')!='PARAMETER_CELL_CANONICAL_V1_3_11':v.err('parameter cell canonicalization version mismatch')
        rng=obj.get('rng_contract',{})
        if rng.get('seed_algorithm')!='SHA256_DOMAIN_SEPARATED_CANONICAL_V1_3_11' or rng.get('seed_derivation_algorithm')!='SHA256_DOMAIN_SEPARATED_CANONICAL_V1_3_11':v.err('generator RNG algorithm mismatch')
        ...
        rng=obj.get('rng_contract',{})
        if rng.get('seed_algorithm')!='SHA256_DOMAIN_SEPARATED_CANONICAL_V1_3_11' or rng.get('seed_derivation_algorithm')!='SHA256_DOMAIN_SEPARATED_CANONICAL_V1_3_11':v.err('simulation RNG algorithm mismatch')
...
def _validate_formal_values(f,root,schema,v):
    ...
    for obj,label in [(pre,'pre-core'),(seed,'seed')]:
        ...
        if obj.get('seed_derivation_algorithm')!='SHA256_DOMAIN_SEPARATED_CANONICAL_V1_3_11':v.err(f'{label} seed derivation algorithm mismatch')
    ...
    if oc.get('gate_evidence_bundle_domain')!='ASAP_BLOCK:GATE_EVIDENCE_BUNDLE:v1.3.11':v.err('gate evidence bundle domain mismatch')
...
def validate_formal(root,schema,canon,v):
    ...
    if not isinstance(fh,str):v.err('formal contract not frozen')
        ...
        try:
            if canonical_object_self_hash(f,'contract_metadata.formal_contract_hash','ASAP_BLOCK:FORMAL_CONTRACT:v1.3.11')!=fh:v.err('formal contract self-hash mismatch')
    ...
    # Child contracts.
    child_specs=[
        ('generator','generator_contract.yaml','generator_contract_hash','ASAP_BLOCK:GENERATOR_CONTRACT:v1.3.11','ASAP_BLOCK_generator_contract_template_v1_3_11.yaml','generator_template_sha256'),
        ('simulation','simulation_contract.yaml','simulation_contract_hash','ASAP_BLOCK:SIMULATION_CONTRACT:v1.3.11','ASAP_BLOCK_simulation_contract_template_v1_3_11.yaml','simulation_template_sha256'),
        ('trace_generator','trace_generator_contract.yaml','trace_generator_contract_hash','ASAP_BLOCK:TRACE_GENERATOR_CONTRACT:v1.3.11','ASAP_BLOCK_trace_generator_contract_template_v1_3_11.yaml','trace_template_sha256')]
    ...
    if seed.get('seed_derivation_algorithm')!='SHA256_DOMAIN_SEPARATED_CANONICAL_V1_3_11':v.err('unexpected seed algorithm')
    # Internal hash DAG.
    ...
    pch=domain_hash('ASAP_BLOCK:PLAN_CONTEXT:v1.3.11',pp)
    ...
    if not isinstance(fg,dict) or not isinstance(fg.get('cells'),list) or not fg['cells']:v.err('formal grid cells missing')
        ...
        for cell in fg['cells']:
            ...
            cid=domain_hash('ASAP_BLOCK:PARAMETER_CELL:v1.3.11',cell['parameters'])
    ...
    fgh=domain_hash('ASAP_BLOCK:FORMAL_GRID:v1.3.11',fg)
    ...
    sch=domain_hash('ASAP_BLOCK:SEED_CONTEXT:v1.3.11',scpre)
...
def seed_value(formal,scope,replicate):
    ...
    digest=hashlib.sha256(b'ASAP_BLOCK:DERIVED_SEED:v1.3.11\x00'+canonical_json_bytes(pre)).digest()
...
def validate_plan(root,rows,formal,canon,v,profile):
    ...
    for r in plans:
        ...
        calc=domain_hash(f'ASAP_BLOCK:REQUEST_PAYLOAD:{typ}:v1.3.11',pre)
        ...
        if domain_hash('ASAP_BLOCK:REQUEST:v1.3.11',rp)!=r['request_id']:v.err(f'request_id mismatch:{r["request_id"]}')
        ...
        if domain_hash('ASAP_BLOCK:EXPECTED_OUTPUT:v1.3.11',op)!=r['expected_output_id']:v.err(f'expected_output_id mismatch:{r["request_id"]}')
        ...
        if typ in SEEDED_TYPES:
            ...
            scope=domain_hash('ASAP_BLOCK:SEED_SCOPE:v1.3.11',scope_pre)
    ...
    if domain_hash('ASAP_BLOCK:SEED_SET:v1.3.11',seed_rows)!=formal['seed_contract']['formal_seed_set_hash']:v.err('formal_seed_set_hash mismatch')
    ...
    if domain_hash('ASAP_BLOCK:RUN_PLAN_BUNDLE:v1.3.11',bundle_pre)!=formal['run_plan_contract']['run_plan_bundle_hash']:v.err('run_plan_bundle_hash mismatch')
...
def execution_and_outputs(root,rows,formal,canon,plans,schema,v,profile):
    ...
    for rid,pr in plans.items():
        ...
        if final.get(rid)=='FINISHED':
            ...
            bh=domain_hash(f'ASAP_BLOCK:OUTPUT_BUNDLE:{pr["request_type"]}:v1.3.11',pre)
...
def validate_analysis_certification_states(arows,trs,v):
    ...
    for aid,a in arows.items():
        ...
        if a.get('analysis_certification_status') not in {'CERTIFIED_TASKSET','DIAGNOSTIC_ONLY_NOT_CERTIFIED','NOT_CERTIFIED','NOT_APPLICABLE'}:
            v.err(f'forbidden v1.3.11 analysis certification status:{aid}')
...
def _bind_fixture_carry_in_hashes(trs):
    ...
    for target in trs['loc']:
        ...
        target['carry_in_vector_hash']=domain_hash('ASAP_BLOCK:CARRY_IN_VECTOR:v1.3.11',pre)
...
def recompute_taskset_hashes(ts,tasks):
    ...
    return {'taskset_semantic_hash':domain_hash('ASAP_BLOCK:TASKSET_SEMANTIC:v1.3.11',{'M':ts['M'],'n':ts['n'],'tasks':sem}),'priority_rank_hash':domain_hash('ASAP_BLOCK:PRIORITY_RANK:v1.3.11',pr),'power_vector_raw_hash':domain_hash('ASAP_BLOCK:POWER_VECTOR_RAW:v1.3.11',raw),'power_vector_analysis_hash':domain_hash('ASAP_BLOCK:POWER_VECTOR_ANALYSIS:v1.3.11',ana)}
...
def validate_lineage(rows,schema,formal,canon,plans,v,profile):
    ...
    for aid,a in arows.items():
        ...
        for r in rr:
            ...
            calc=domain_hash('ASAP_BLOCK:TASK_RESULT:v1.3.11',{k:(None if v=='' else v) for k,v in r.items() if k!='task_result_hash'})
    ...
    for r in rows.get('rta_dependency_records.csv',[]):
        ...
        calc=domain_hash('ASAP_BLOCK:DEPENDENCY_RECORD:v1.3.11',{k:(None if v=='' else v) for k,v in r.items() if k!='dependency_record_hash'})
    ...
    for key,ds in deps_by_target.items():
        ...
        h=domain_hash('ASAP_BLOCK:CARRY_IN_VECTOR:v1.3.11',pre)
    ...
    for rid,rs in rel_sets.items():
        ...
        h=domain_hash('ASAP_BLOCK:RELEASE_TRACE:v1.3.11',[{k:r[k] for k in ['job_id','task_id','release_time','execution_demand','absolute_deadline']} for r in rr])
    ...
    for hid,hs in har_sets.items():
        ...
        raw=domain_hash('ASAP_BLOCK:HARVEST_TRACE_RAW:v1.3.11',[{'tick':r['tick'],'H_raw':r['H_raw']} for r in rr]);ana=domain_hash('ASAP_BLOCK:HARVEST_TRACE_ANALYSIS:v1.3.11',[{'tick':r['tick'],'H_analysis':r['H_analysis']} for r in rr])
    ...
    for tn,index,ctype in [('service_trace_checks.csv',service,'SERVICE'),('e0_trace_certificate_checks.csv',e0,'E0'),('analysis_simulation_compatibility_checks.csv',compat,'COMPATIBILITY')]:
        ...
        for r in index.values():
            ...
            if r['check_role']=='FORMAL_PRIMARY':
                ...
                exp=domain_hash(f'ASAP_BLOCK:FORMAL_PRIMARY:{ctype}:v1.3.11',{'simulation_run_id':r['simulation_run_id'],'analysis_run_id':r['analysis_run_id'],'check_type':ctype,'formal_contract_hash':r.get('formal_contract_hash') or None})
