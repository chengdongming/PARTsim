...
EVIDENCE_DOMAIN='ASAP_BLOCK:GATE_EVIDENCE_BUNDLE:v1.3.11'
...
def validate_report(report_path:Path,formal_path:Path|None=None,template_mode:bool=False)->list[str]:
    ...
    if meta.get('canonical_serialization_file')!='ASAP_BLOCK_canonical_serialization_v1_3_11.yaml': errors.append('canonical serialization file mismatch')
    ...
    if not template_mode:
        if not _is_hash(expected_hash): errors.append('acceptance_report_hash missing or invalid')
            ...
            actual=canonical_object_self_hash(report,'acceptance_report_metadata.acceptance_report_hash','ASAP_BLOCK:ACCEPTANCE_REPORT:v1.3.11')
