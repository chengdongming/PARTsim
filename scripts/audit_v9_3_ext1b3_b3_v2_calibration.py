...
def _dataset_integrity_audit(
    ...
    unknown_attempt_statuses = sorted(
        {
            str(row.get("attempt_status")) for row in attempts
        }.difference({"ACCEPTED", "REJECTED"})
    )
    if unknown_attempt_statuses:
        errors.append(f"unknown generation attempt statuses: {unknown_attempt_statuses}")
    accepted_keys: list[tuple[str, str, int]] = []
    for row in attempts:
        try:
            logical = _integer(row.get("logical_taskset_index"), "logical index")
            attempt = _integer(row.get("attempt_index"), "attempt index")
            source = _integer(row.get("source_taskset_index"), "source index")
            if not 0 <= attempt < FROZEN_STRUCTURAL_RETRY_LIMIT:
                errors.append("generation attempt is outside structural_retry_limit")
            if source != logical * FROZEN_STRUCTURAL_RETRY_LIMIT + attempt:
                errors.append("generation source-index rule mismatch")
            if _integer(row.get("source_index"), "compatibility source index") != source:
                errors.append("generation compatibility source_index mismatch")
            if str(row.get("attempt_status")) == "ACCEPTED":
                accepted_keys.append((
                    str(row.get("scenario_cell_id")),
                    _canonical_fraction(
                        row.get("normalized_utilization"), "attempt utilization"
                    ),
                    logical,
                ))
    ...
    if len(accepted_keys) != len(set(accepted_keys)):
        errors.append("a calibration logical unit has multiple accepted samples")
