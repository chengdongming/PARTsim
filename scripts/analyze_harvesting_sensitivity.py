#!/usr/bin/env python3
"""Aggregate synthetic harvesting solar-time sensitivity runs."""
...
from scripts.run_harvesting_sensitivity import LIMITATION


BY_SEED_FIELDS = [
    'config_group_id', 'config_id', 'scheduler', 'algorithm_display_name',
    'normalized_utilization',
    'solar_time_ms', 'harvesting_profile', 'seed_base', 'num_tasksets',
...
SUMMARY_FIELDS = [
    ...
    'normalized_utilization',
    'solar_time_ms', 'harvesting_profile', 'mean_acceptance_ratio',
...
def summarize_harvesting(manifest_path, allow_legacy=False):
    manifest_path = Path(manifest_path)
    manifest = read_csv(manifest_path)
    required = {'run_dir', 'solar_time_ms', 'seed_base'}
    ...
    for _, entry in manifest.iterrows():
        ...
        for _, observation in observations.iterrows():
            rows.append({
                ...
                'algorithm_display_name': observation[
                    'algorithm_display_name'
                ],
                'normalized_utilization': observation[
                    'normalized_utilization'
                ],
                'solar_time_ms': int(entry['solar_time_ms']),
    ...
    if not duplicate.empty:
        raise ValueError(
            'duplicate_aggregate_result: harvesting sensitivity inputs '
    ...
    group_fields = [
        'config_group_id', 'scheduler', 'algorithm_display_name',
        'normalized_utilization',
        'solar_time_ms', 'harvesting_profile',
    ]
...
def write_harvesting_outputs(manifest_path, output_dir, allow_legacy=False):
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    by_seed, summary = summarize_harvesting(
        manifest_path, allow_legacy=allow_legacy
    )
    by_seed.to_csv(
        output_dir / 'harvesting_sensitivity_by_seed.csv', index=False
    )
    summary.to_csv(
        output_dir / 'harvesting_sensitivity_summary.csv', index=False
    )
    fig, ax = plt.subplots(figsize=(9, 6))
    for (scheduler, solar_time), group in summary.groupby(
        ['scheduler', 'solar_time_ms'], sort=True
    ):
        group = group.sort_values('normalized_utilization')
        ax.plot(
            group['normalized_utilization'],
            group['mean_acceptance_ratio'],
            marker='o', label='{} solar={}'.format(scheduler, solar_time),
        )
    ...
    fig.tight_layout()
    fig.savefig(output_dir / 'harvesting_sensitivity_plot.png', dpi=200)
    plt.close(fig)
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description='Analyze solar-time sensitivity. ' + LIMITATION
    )
