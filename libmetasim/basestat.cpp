...
namespace MetaSim {
    ...
    const char *const EFFECTIVE_ATTACH = "Unhandled attach()!";

    const char *const GET = "Can't get statistics before end of simulation\n";

    const char *const NO_INIT = "Statistics not initialized";

    const char *const NEED_3 = "Need at least 3 run to evaluate statistic";

    static const char *const TOO_MUCH_RUNS = "Too many runs! \n";

    static double t1_table[30][2] = {
        {6.314, 12.706}, {2.920, 4.303}, {2.353, 3.182}, {2.132, 2.776},
        {2.015, 2.571},  {1.943, 2.447}, {1.895, 2.365}, {1.860, 2.306},
        {1.833, 2.262},  {1.812, 2.228}, {1.796, 2.201}, {1.782, 2.179},
        {1.771, 2.160},  {1.768, 2.145}, {1.753, 2.131}, {1.746, 2.120},
        {1.740, 2.110},  {1.734, 2.101}, {1.729, 2.093}, {1.725, 2.086},
        {1.721, 2.080},  {1.717, 2.074}, {1.714, 2.069}, {1.711, 2.064},
        {1.708, 2.060},  {1.706, 2.056}, {1.703, 2.052}, {1.701, 2.048},
        {1.699, 2.045},  {1.697, 2.042}};

    BaseStat::BaseStat(std::string n) : _name(n) {
    ...
    void BaseStat::printAll() {
        for_each(BaseStat::begin(), BaseStat::end(),
                 std::mem_fn(&BaseStat::print));
    }

    void BaseStat::print() {
        std::cout << "[" << getName() << "]:" << getMean()
    ...
    */
    ...
    void TableOutput::write(string message) {
        if (!_created)
            throw BaseExc("Init function not called", "TableOutput",
                          "basestat.cpp");
