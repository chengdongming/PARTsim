#ifndef GPFP_CBPP_SCHEDULER_HPP
#define GPFP_CBPP_SCHEDULER_HPP
...
namespace RTSim {
    ...
    // 任务激活仿真事件类声明
    // =====================================================
    class CBPPTaskActivationSimEvent : public MetaSim::Event {
    private:
        CBPPScheduler *_scheduler;
        AbsRTTask *_task;
    ...
    public:
        CBPPTaskActivationSimEvent(CBPPScheduler *scheduler,
                                      AbsRTTask *task,
    ...
    // CBPP周期性等待队列检查事件
    // =====================================================
    class CBPPWaitingQueueCheckEvent : public MetaSim::Event {
    private:
        CBPPScheduler *_scheduler;

    public:
        CBPPWaitingQueueCheckEvent(CBPPScheduler *scheduler);
        void doit() override;
    ...
    // V28.14: 能量恢复事件
    // =====================================================
    class CBPPEnergyRecoveryEvent : public MetaSim::Event {
    private:
        CBPPScheduler *_scheduler;

    public:
        CBPPEnergyRecoveryEvent(CBPPScheduler *scheduler);
        void doit() override;
    };

    // ⭐ 周期性能量收集事件
    class CBPPPeriodicEnergyCollectionEvent : public MetaSim::Event {
    private:
        CBPPScheduler *_scheduler;

    public:
        CBPPPeriodicEnergyCollectionEvent(CBPPScheduler *scheduler);
        void doit() override;
    };
    ...
    // CBPPTaskModel 类声明
    // =====================================================
    class CBPPTaskModel : public TaskModel {
    private:
        int _period;
        int _wcet;
    ...
    public:
        CBPPTaskModel(AbsRTTask *t, int period, int wcet,
                     const std::string &workload_type,
                     double energy_coefficient = 1.0,
                     MetaSim::Tick arrival_offset = 0);
        virtual ~CBPPTaskModel();
    ...
    // CBPPScheduler 类声明
    // =====================================================
    class CBPPScheduler : public Scheduler {
    private:
        // 任务参数结构
        struct TaskParams {
            int period;
        ...
        // ========== 任务管理 ==========
        std::map<AbsRTTask *, CBPPTaskModel *> _task_models;
        ...
        std::vector<CBPPTaskActivationSimEvent *> _scheduled_sim_events;
        std::map<int64_t, std::vector<AbsRTTask *>> _scheduled_activations;

        // ========== CBPP不需要分片管理 ==========
        // CBPP使用Tick级抢占，不使用50ms分片
        ...
        // ========== ⭐ CBPP就绪队列 ==========
        std::deque<AbsRTTask *> _ready_queue;  // ⭐ CBPP专用的就绪队列

        // ========== ⭐ CBPP批量调度管理 ==========
        struct BatchScheduleInfo {
            std::vector<AbsRTTask *> batch_tasks;  // 当前批量任务集合
            double total_energy_needed;            // 批量总能耗
            int batch_size;                        // 批量大小K
            MetaSim::Tick batch_decision_time;     // 批量决策时间
            bool is_scheduled;                     // 是否已调度

            BatchScheduleInfo() : total_energy_needed(0.0), batch_size(0),
                                  batch_decision_time(0), is_scheduled(false) {}
        };

        BatchScheduleInfo _current_batch;  // 当前批量调度信息
        ...
        CBPPEnergyRecoveryEvent *_recovery_event;   // 能量恢复事件
        CBPPPeriodicEnergyCollectionEvent *_periodic_collection_event;  // ⭐ 周期性能量收集事件
        ...
        // ========== ⭐ CBPP批量调度核心方法 ==========
        int calculateBatchSize();  // 计算批量大小 K = min(空闲核心, 就绪任务数)
        std::vector<AbsRTTask *> selectCooperativeBatch(int batch_size);  // 选择前K个最高优先级任务
        double calculateBatchTotalEnergy(const std::vector<AbsRTTask *> &batch);  // 计算批量总能耗
        bool canScheduleBatchWithEnergy(const std::vector<AbsRTTask *> &batch,
                                        MetaSim::Tick current_time);  // 批量能量判断
        bool consumeBatchEnergy(const std::vector<AbsRTTask *> &batch);  // 批量能量预扣
        void scheduleBatch(const std::vector<AbsRTTask *> &batch);  // 批量调度
        void clearBatchSchedule();  // 清空批量信息
        ...
        CBPPTaskModel *getTaskModel(AbsRTTask *task);
    ...
    public:
        // 构造函数/析构函数
        CBPPScheduler();
        CBPPScheduler(const std::vector<std::string> &params);
        virtual ~CBPPScheduler();

        // 工厂方法
        static std::unique_ptr<CBPPScheduler>
        ...
        // 友元类声明
        friend class CBPPTaskActivationSimEvent;
        friend class CBPPEnergyRecoveryEvent;
        friend class CBPPPeriodicEnergyCollectionEvent;  // ⭐ 周期性能量收集
        // CBPP不需要分片事件：使用Tick级抢占
        friend class CBPPWaitingQueueCheckEvent;
...
// 工厂注册
namespace RTSim {
    static registerInFactory<RTSim::Scheduler, RTSim::CBPPScheduler>
        registerCBPPScheduler("gpfp_cbpp");
}

#endif // GPFP_CBPP_SCHEDULER_HPP
