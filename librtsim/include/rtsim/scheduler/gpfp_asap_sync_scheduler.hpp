...
namespace RTSim {
    ...
    // =====================================================
    class ASAPSyncTickEvent : public MetaSim::Event {
    private:
        ASAPSyncScheduler *_scheduler;

    public:
        ASAPSyncTickEvent(ASAPSyncScheduler *scheduler);
        void doit() override;
    };
    ...
    // =====================================================
    class ASAPSyncEnergyCheckEvent : public MetaSim::Event {
    private:
        ASAPSyncScheduler *_scheduler;
        AbsRTTask *_task;
        CPU *_cpu;
    ...
    public:
        ASAPSyncEnergyCheckEvent(ASAPSyncScheduler *scheduler, AbsRTTask *task, CPU *cpu);
        void doit() override;
    ...
    // =====================================================
    // ASAPSyncTaskModel 类声明
    // =====================================================
    class ASAPSyncTaskModel : public TaskModel {
    private:
        int _period;
        int _wcet;
    ...
    public:
        ASAPSyncTaskModel(AbsRTTask *t, int period, int wcet,
                      const std::string &workload_type,
                      double energy_coefficient = 1.0,
                      MetaSim::Tick arrival_offset = 0);
        virtual ~ASAPSyncTaskModel();
    ...
    // =====================================================
    // ASAPSyncScheduler 类声明 - BTIE批量调度算法
    // =====================================================
    class ASAPSyncScheduler : public Scheduler, public EnergyInfoProvider {
    private:
        // ========== 核心配置参数 ==========
        ...
        // ========== Tick事件 ==========
        ASAPSyncTickEvent *_tick_event;
        bool _first_tick_scheduled;  // 标记第一个tick是否已调度

        // ========== 任务管理 ==========
        std::map<AbsRTTask *, ASAPSyncTaskModel *> _task_models;
    ...
    public:
        // 构造函数/析构函数
        ASAPSyncScheduler();
        ASAPSyncScheduler(const std::vector<std::string> &params);
        virtual ~ASAPSyncScheduler();

        // 工厂方法
        static std::unique_ptr<ASAPSyncScheduler>
...
// 工厂注册
namespace RTSim {
    static registerInFactory<RTSim::Scheduler, RTSim::ASAPSyncScheduler>
        registerASAPSyncScheduler("gpfp_asap_sync");
}
