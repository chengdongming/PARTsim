#ifndef GPFP_EFPP_SCHEDULER_HPP
#define GPFP_EFPP_SCHEDULER_HPP
...
namespace RTSim {
    ...
    // 任务激活仿真事件类声明
    // =====================================================
    class EFPPTaskActivationSimEvent : public MetaSim::Event {
    private:
    ...
    public:
        EFPPTaskActivationSimEvent(EFPFPScheduler *scheduler,
                                      AbsRTTask *task,
    ...
    // EPP周期性等待队列检查事件
    // =====================================================
    class EFPPWaitingQueueCheckEvent : public MetaSim::Event {
    private:
        EFPFPScheduler *_scheduler;

    public:
        EFPPWaitingQueueCheckEvent(EFPFPScheduler *scheduler);
        void doit() override;
    ...
    // V28.14: 能量恢复事件
    // =====================================================
    class EFPPEnergyRecoveryEvent : public MetaSim::Event {
    private:
        EFPFPScheduler *_scheduler;

    public:
        EFPPEnergyRecoveryEvent(EFPFPScheduler *scheduler);
        void doit() override;
    };

    // ⭐ 周期性能量收集事件
    class EFPPPeriodicEnergyCollectionEvent : public MetaSim::Event {
    private:
        EFPFPScheduler *_scheduler;

    public:
        EFPPPeriodicEnergyCollectionEvent(EFPFPScheduler *scheduler);
        void doit() override;
    };
    ...
    // EFPPTaskModel 类声明
    // =====================================================
    class EFPPTaskModel : public TaskModel {
    private:
        int _period;
        int _wcet;
    ...
    public:
        EFPPTaskModel(AbsRTTask *t, int period, int wcet,
                     const std::string &workload_type,
                     double energy_coefficient = 1.0,
                     MetaSim::Tick arrival_offset = 0);
        virtual ~EFPPTaskModel();
    ...
    // EFPFPScheduler 类声明
    // =====================================================
    class EFPFPScheduler : public Scheduler {
    private:
        // 任务参数结构
        struct TaskParams {
            int period;
        ...
        // ========== 任务管理 ==========
        std::map<AbsRTTask *, EFPPTaskModel *> _task_models;
        ...
        std::vector<EFPPTaskActivationSimEvent *> _scheduled_sim_events;
        ...
        EFPPEnergyRecoveryEvent *_recovery_event;   // 能量恢复事件
        EFPPPeriodicEnergyCollectionEvent *_periodic_collection_event;  // ⭐ 周期性能量收集事件
        ...
        EFPPTaskModel *getTaskModel(AbsRTTask *task);
    ...
    public:
        // 构造函数/析构函数
        EFPFPScheduler();
        EFPFPScheduler(const std::vector<std::string> &params);
        virtual ~EFPFPScheduler();

        // 工厂方法
        static std::unique_ptr<EFPFPScheduler>
        ...
        // 友元类声明
        friend class EFPPTaskActivationSimEvent;
        friend class EFPPEnergyRecoveryEvent;
        friend class EFPPPeriodicEnergyCollectionEvent;  // ⭐ 周期性能量收集
        // EPP不需要分片事件：使用Tick级抢占
        friend class EFPPWaitingQueueCheckEvent;
...
// 工厂注册
namespace RTSim {
    static registerInFactory<RTSim::Scheduler, RTSim::EFPFPScheduler>
        registerEFPFPScheduler("gpfp_efpp");
}

#endif // GPFP_EFPP_SCHEDULER_HPP
