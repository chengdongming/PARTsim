...
namespace RTSim {
    ...
    private:
        ...
        void scheduleWithoutEnergyManagement();
        void debugTimeAnalysis();
        ...
        // 添加能量验证函数
        bool validateRecoveryParameters(double required_energy,
        ...
        void recordTaskCompletion(
            AbsRTTask *task, MetaSim::Tick completion_time);

        // 能量管理
        void handleEnergyRecovery(MetaSim::Tick current_time);
        void scheduleRecoveryCheckEvent(int64_t check_time_ms);
    ...
    public:
        ...
        // 调度方法（修复：不标记为override，基类可能没有这个虚函数）
        void schedule();
        ...
        // 友元类声明（修复：确保TaskActivationSimEvent可以访问私有成员）
