#ifndef GPFP_CASCADE_SCHEDULER_HPP
#define GPFP_CASCADE_SCHEDULER_HPP
...
namespace RTSim {
    ...
    // 任务激活仿真事件类声明
    // =====================================================
    class CASCADETaskActivationSimEvent : public MetaSim::Event {
    private:
    ...
    public:
        CASCADETaskActivationSimEvent(GPFPCASCADEScheduler *scheduler,
                                      AbsRTTask *task,
    ...
    // CASCADE任务分片定时器事件
    // =====================================================
    class CASCADESlicingEvent : public MetaSim::Event {
    private:
        GPFPCASCADEScheduler *_scheduler;
        AbsRTTask *_task;

    public:
        CASCADESlicingEvent(GPFPCASCADEScheduler *scheduler, AbsRTTask *task);
        void doit() override;
    ...
    public:
        GPFPCASCADETaskModel(AbsRTTask *t, int period, int wcet,
                             const std::string &workload_type,
                             MetaSim::Tick arrival_offset = 0);
        virtual ~GPFPCASCADETaskModel();
    ...
    // =====================================================
    // GPFPCASCADEScheduler 类声明
    // =====================================================
    class GPFPCASCADEScheduler : public Scheduler {
    private:
        // 任务参数结构
        struct TaskParams {
            int period;
        ...
        // ========== 任务管理 ==========
        std::map<AbsRTTask *, GPFPCASCADETaskModel *> _task_models;
        ...
        std::vector<CASCADETaskActivationSimEvent *> _scheduled_sim_events;
        std::map<int64_t, std::vector<AbsRTTask *>> _scheduled_activations;

        // ========== 任务分片管理 ==========
        std::map<AbsRTTask *, CASCADESlicingEvent *> _active_slicing_events;
        ...
        void parseCASCADEParams(const std::vector<std::string> &params);
        void validateEnergyCalculations();
        void validateConfiguration();
        
        // CASCADE核心算法
        std::vector<AbsRTTask *> performCASCADESchedule(MetaSim::Tick current_time,
                                                       double current_energy);
        void executeCASCADESelectedTasks(const std::vector<AbsRTTask *> &tasks_to_run);
    ...
    public:
        // 构造函数/析构函数
        GPFPCASCADEScheduler();
        GPFPCASCADEScheduler(const std::vector<std::string> &params);
        virtual ~GPFPCASCADEScheduler();

        // 工厂方法
        static std::unique_ptr<GPFPCASCADEScheduler>
        ...
        // ========== 批量插入延迟检查机制（避免优先级反转） ==========
        // 这些方法可以被CASCADE、ASAP、Batch等算法共用
...
// 工厂注册
namespace RTSim {
    static registerInFactory<RTSim::Scheduler, RTSim::GPFPCASCADEScheduler>
        registerGPFPCASCADE("gpfp_cascade");
}

#endif // GPFP_CASCADE_SCHEDULER_HPP
