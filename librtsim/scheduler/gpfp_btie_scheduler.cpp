...
namespace RTSim {
    ...
    void BTIEScheduler::performTickScheduling() {
        ...
        if (_current_energy >= total_energy - EPSILON) {
            ...
            // 能量不足：不调度任何新任务，让当前运行的任务自然结束
            _batch_scheduled_this_tick = false;
            _current_batch_tasks.clear();
            _current_batch_size = 0;
            _stats.total_batch_skipped++;

            // ⭐ BTIE核心：不suspend运行中任务，而是让它们在本ms结束后自然停止
            // getTaskN()会因为_batch_scheduled_this_tick=false而不返回任何新任务
            // 当前运行的任务会在本tick结束时自动结束（不续期）
            ...
            SCHEDULER_LOG_INFO(std::string("❌ [BTIE] 能量不足，本tick不调度任务: ") +
                              ...
                              " 运行中=" + std::to_string(running_tasks.size()));
        }
    ...
    AbsRTTask *BTIEScheduler::getTaskN(unsigned int n) {
        // ⭐ BTIE核心：检查本tick是否批量调度成功
        // 如果能量不足导致批量调度失败，不返回任何任务（包括运行中任务）
        if (!_batch_scheduled_this_tick) {
            SCHEDULER_LOG_DEBUG(std::string("🚫 [BTIE] getTaskN(") + std::to_string(n) + "): 批量调度失败，不调度任务");
            return nullptr;
        }
    ...
    void BTIEScheduler::checkAndInterruptRunningTasks() {
        // ⭐ BTIE修复：能量扣除已移至performTickScheduling的批量决策中
        // 此函数现在留空，所有能量管理在批量调度时统一完成
        SCHEDULER_LOG_DEBUG("🔍 [BTIE] checkAndInterruptRunningTasks: 能量检查已移至批量决策");
        
        // 此函数保留以保持接口兼容，但不再执行能量管理操作
    }
