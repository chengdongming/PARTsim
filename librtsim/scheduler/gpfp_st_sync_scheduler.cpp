...
namespace RTSim {
    ...
    void STSyncScheduler::suspendBatchForInsufficientEnergy(const std::vector<AbsRTTask *> &tasks,
        ...
        Tick wake_time = calculateGroupWakeTime(min_slack);
    ...
    // The per-tick path rechecks battery state after actual harvested energy
    // arrives.  This event is only the existing slack-boundary fallback and
    // never predicts future source output.
    MetaSim::Tick STSyncScheduler::calculateGroupWakeTime(MetaSim::Tick group_slack) {
        Tick current_time = SIMUL.getTime();

        // 计算Slack归零时刻
        int64_t slack_deadline = static_cast<int64_t>(group_slack);

        int64_t wake_offset = slack_deadline;
