...
namespace RTSim {
    ...
    void STBlockScheduler::performTickScheduling() {
        ...
        const HarvestResult harvest_result =
            _harvest_runtime.applyAtDecisionTime(
                ...
                _current_energy,
                _max_energy);
        const double preclip_observation_energy =
            harvest_result.clipped_j > 0.0
                ? harvest_result.battery_before_j + harvest_result.offered_j
                : harvest_result.battery_after_j;
                ...
                if (_trace_logger &&
                    ...
                    if (_st_charge_blocked_task &&
                        std::find(active_tasks.begin(), active_tasks.end(),
                                  _st_charge_blocked_task) ==
                            active_tasks.end()) {
                        active_tasks.push_back(_st_charge_blocked_task);
                    }
                    ...
                    const std::size_t observed_processors =
                        native_processors > 0
                            ? native_processors
                            : std::max<std::size_t>(
                    ...
                    for (AbsRTTask *task : active_tasks) {
                        ...
                        if (task == _st_charge_blocked_task) {
                            after_blocked = true;
                        }
                    }
        ...
        if (_energy_depleted && _current_energy < 0.000001) {
            if (_trace_logger &&
                ...
                std::map<AbsRTTask *, double> costs;
                std::map<AbsRTTask *, DecisionExclusionReason> reasons;
                for (std::size_t i = 0; i < active_tasks.size(); ++i) {
                    costs[active_tasks[i]] =
                        getConfiguredUnitEnergyForTask(active_tasks[i]);
                    reasons[active_tasks[i]] = i == 0
        ...
        if (_trace_logger &&
            ...
            std::map<AbsRTTask *, double> costs;
            std::map<AbsRTTask *, DecisionExclusionReason> reasons;
            bool after_blocking_task = false;
            const bool charge_wait = !timing_wait_tasks.empty();
            for (AbsRTTask *task : active_tasks) {
                costs[task] = getConfiguredUnitEnergyForTask(task);
                if (actual.count(task) > 0) {
                    reasons[task] = DecisionExclusionReason::None;
                } else if (task == blocking_task) {
                    reasons[task] = charge_wait
                        ? DecisionExclusionReason::StEnergyChargeWait
                        : DecisionExclusionReason::DirectEnergyShortage;
                    after_blocking_task = true;
                } else if (after_blocking_task) {
                    reasons[task] =
                        DecisionExclusionReason::BlockHeadOfLine;
                } else if (charge_wait) {
                    reasons[task] =
    ...
    AbsRTTask *STBlockScheduler::getFirst() {
        ...
        if (_current_energy < unit_energy) {
            ...
            // The per-tick path rechecks battery state after actual harvested
            // energy arrives.  This event is only the existing slack-boundary
            // fallback; it must not predict future source output.
            Tick wake_time = computeSafeWakeTimeFromOffset(slack_ms);
    ...
    MetaSim::Tick STBlockScheduler::calculateMinSlack() {
        ...
        Tick min_slack;
        if (!found_valid_task) {
            // No task slack is available. Recheck on the next tick after
            // actual interval energy arrives; never predict a full-battery
            // time from an average harvesting rate.
            min_slack = Tick(1);
            SCHEDULER_LOG_INFO(
                "🧮 [ST-Block] 没有活跃任务，下一tick重新检查充电状态");
        } else {
