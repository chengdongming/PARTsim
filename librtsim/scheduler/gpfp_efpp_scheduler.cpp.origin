...
namespace RTSim {
    ...
    AbsRTTask *EFPFPScheduler::getFirst() {
        ...
        if (!first_task) {
            SCHEDULER_LOG_DEBUG("📭 [EFPP] getFirst: 队列首任务为空");
            return nullptr;
        }
        ...
        if (!can_schedule) {
            SCHEDULER_LOG_INFO(std::string("❌ [EFPP] getFirst: 前瞻性判断能量不足，停止调度") +
        ...
        if (!consumeEnergy(energy_needed, task_name)) {
            ...
            SCHEDULER_LOG_WARNING(std::string("⚠️ [EFPP] getFirst: 前瞻性判断通过但实际能量不足") +
                                 " 任务=" + task_name +
        ...
        // 6. ✅ 能量已扣减，返回任务
        SCHEDULER_LOG_INFO(std::string("✅ [EFPP] getFirst: 能量已扣减，返回任务: ") +
                          task_name +
                          " 当前能量: " + std::to_string(_current_energy) + "J");
    ...
    AbsRTTask *EFPFPScheduler::getTaskN(unsigned int n) {
        ...
        for (size_t i = 0; i < _ready_queue.size(); ++i) {
            ...
            if (ready_index == n) {
                ...
                if (!consumeEnergy(energy_needed, task_name)) {
                    // ⭐ 关键修复：扣减失败时，启动能量恢复并返回nullptr
                    SCHEDULER_LOG_WARNING(std::string("⚠️ [EFPP] getTaskN: 前瞻性判断通过但实际能量不足") +
                                         " 任务=" + task_name +
                                         " 需要=" + std::to_string(energy_needed) + "J" +
                                         " 当前=" + std::to_string(_current_energy) + "J");
        ...
        // 没有找到第n个未dispatch的任务
        SCHEDULER_LOG_DEBUG("📭 [EFPP] getTaskN: 未找到第" + std::to_string(n) + "个未dispatch的任务");
        return nullptr;
    }
