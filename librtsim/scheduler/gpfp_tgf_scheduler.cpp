...
namespace RTSim {
    ...
    void TGFEnergyCheckEvent::doit() {
        ...
        if (current_energy < unit_energy - EPSILON) {
            // 能量不足，停止续期并中断任务
            SCHEDULER_LOG_INFO(std::string("⚡ [TGF] 续期能量不足，中断任务: ") +
    ...
    void TGFScheduler::performTickScheduling() {
        ...
        // 3. 如果有kernel，循环触发dispatch直到填满所有CPU
        if (!_kernel) {
        ...
        if (_kernel) {
            SCHEDULER_LOG_DEBUG("🔔 [TGF] performTickScheduling: 开始循环调度填满所有CPU");
    ...
    AbsRTTask *TGFScheduler::getTaskN(unsigned int n) {
        ...
        for (size_t i = 0; i < _ready_queue.size(); ++i) {
            ...
            if (ready_index == n) {
                ...
                SCHEDULER_LOG_INFO(std::string("✅ [TGF] 预扣能量并调度任务: ") + getTaskName(task) +
