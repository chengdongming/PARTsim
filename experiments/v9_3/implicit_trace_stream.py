...
def _consume_json_value(reader: _ChunkReader) -> str:
...
def _decode_event(raw: str) -> dict[str, Any]:
    value = _decode_complete(raw, "event")
    if not isinstance(value, dict):
        raise ValueError("event is not an object")
    # Re-decode with the duplicate-key hook; this is intentionally per-event.
    try:
...
def _scan_document(reader: _ChunkReader) -> Iterator[dict[str, Any]]:
    reader.skip_whitespace()
    if reader.take() != "{":
...
def open_strict_stream(path: Path) -> tuple[Mapping[str, Any], Iterator[dict[str, Any]]]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        metadata = _finish_scan(_scan_document(_ChunkReader(handle)))

    def events() -> Iterator[dict[str, Any]]:
        with path.open("r", encoding="utf-8") as handle:
            scanner = _scan_document(_ChunkReader(handle))
            yield from scanner
