...
_TABLES: Dict[str, Tuple[str, ...]] = {
    ...
    "formal_rta_requests.csv": COMMON + (
        "analysis_id", "request_id", "execution_run_id", "cell_id",
        "taskset_skeleton_id", "taskset_id", "taskset_hash", "method",
        ...
        "source_analysis_id", "request_status",
    ),
    "formal_rta_attempts.csv": COMMON + (
        "attempt_id", "analysis_id", "execution_run_id", "attempt_number",
    ...
    "formal_rta_taskset_results.csv": COMMON + (
        ...
        "taskset_skeleton_id", "taskset_id", "taskset_hash", "method",
        "method_role", "carry_policy", "exact_e0", "service_identity",
        "power_vector_hash", "solver_status", "taskset_certification_status",
        ...
        "exact_result_hash", "source_analysis_id", "fallback_used",
        "axis", "axis_value", "normalized_utilization",
    ),
    "formal_rta_task_results.csv": COMMON + (
        "analysis_id", "taskset_skeleton_id", "taskset_id", "method",
        "exact_e0", "task_id", "priority_rank", "task_solver_status",
        "task_certification_status", "candidate_response_time", "D",
        "checked_w_count", "checked_q_count", "checked_h_count",
        "failure_reason", "exact_task_result_hash",
    ),
    ...
    "formal_dependencies.csv": COMMON + (
        "analysis_id", "source_analysis_id", "relation", "source_core",
        "target_core", "taskset_id", "method", "exact_e0",
        "source_exact_input_identity", "target_exact_input_identity",
        "source_plan_sha256", "source_closure_sha256", "dependency_status",
        "fallback_used",
    ),
    ...
    "formal_simulation_runs.csv": COMMON + (
        "simulation_id", "execution_run_id", "taskset_skeleton_id", "taskset_id",
        ...
        "physical_initial_energy", "service_harvest_identity", "trace_contract",
        "trace_path", "trace_sha256", "simulation_status", "deadline_miss_count",
        "max_observed_response", "release_audit_id", "no_overflow_evidence_id",
        "validated_simulation_evidence_id", "applicability_track",
    ),
    "formal_simulation_task_results.csv": COMMON + (
        "simulation_id", "taskset_id", "task_id", "priority_rank",
    ...
    "formal_simulation_job_results.csv": COMMON + (
        "simulation_id", "taskset_id", "task_id", "job_index", "release_time",
    ...
    "formal_applicability.csv": COMMON + (
        "comparison_id", "analysis_id", "simulation_id", "taskset_id", "method",
