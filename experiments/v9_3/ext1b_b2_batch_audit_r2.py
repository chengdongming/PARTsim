"""Closed retained-trace observation contract for EXT-1B2 screening R2.

R1 remains immutable in :mod:`ext1b_b2_batch_audit`.  This module consumes the
same schema-v2 traces plus additive exact-energy strings emitted by the R2
binary.  Legacy R1 traces are accepted only through the documented six-
significant-digit serialization error bound, so the failed R1 corpus can be
used as a read-only regression input without retrospectively selecting from it.
"""
...
from . import ext1b_b2_batch_audit as r1
...
NATIVE_ENERGY_EPSILON_MJ = r1.NATIVE_ENERGY_EPSILON_MJ
PREDECISION_FINGERPRINT_DOMAIN = (
    "ASAP_BLOCK:V9.3:EXT1B:B2:R2:PREDECISION_STATE_FINGERPRINT:v1"
)
...
class B2R2BatchTraceError(r1.B2BatchTraceError):
    """R2 trace or exact-energy evidence is malformed."""


def _exact_value(value: Any, label: str) -> float:
    if not isinstance(value, str) or not value:
        raise B2R2BatchTraceError(f"{label} exact energy must be text")
    return r1._finite(value, label)


def _normalize_exact_energy(document: Mapping[str, Any]) -> tuple[Dict[str, Any], bool]:
    """Replace rounded compatibility numbers with additive exact strings."""

    normalized = deepcopy(document)
    used_exact = False
    events = normalized.get("events")
    if not isinstance(events, list):
        return normalized, used_exact
    for event_index, event in enumerate(events):
        if not isinstance(event, dict):
            continue
        for field in _EVENT_ENERGY_FIELDS:
            exact_field = f"{field}_exact"
            if exact_field in event:
        ...
        for array_field in _JOB_ARRAY_FIELDS:
            ...
            for job_index, job in enumerate(jobs):
                if not isinstance(job, dict):
                    continue
                if "task_unit_energy_mJ_exact" in job:
...
def _legacy_six_significant_digit_error(value: float) -> float:
    """Maximum absolute rounding error of C++ defaultfloat precision six."""
...
def _legacy_residual_mismatch_is_rounding_only(
    row: Mapping[str, Any], event: Mapping[str, Any],
) -> bool:
    observed = event.get("residual_energy_after_continuation_reservation_mJ")
    available = row.get("available_energy_mJ")
    continuation = row.get("continuation_required_energy_mJ")
    if not all(isinstance(value, (int, float)) for value in (
        observed, available, continuation,
    )):
        return False
    expected = max(0.0, float(available) - float(continuation))
    bound = sum(_legacy_six_significant_digit_error(float(value)) for value in (
        observed, available, continuation,
    )) + 8.0 * sys.float_info.epsilon * max(1.0, abs(expected))
    return abs(float(observed) - expected) <= bound


def _job_identity(event: Mapping[str, Any]) -> str | None:
    return r1._event_job_id(event)
...
def _canonical_number(value: Any) -> str:
    number = r1._finite(value, "state fingerprint numeric field")
...
def _predecision_material(
    ...
    errors: list[str] = []
    ready_ids, ready_energy = r1._jobs(event.get("ready_jobs"), "ready_jobs", errors)
    if errors:
        raise B2R2BatchTraceError("state fingerprint lacks valid ready-job evidence")
    jobs = r1._job_payloads(event.get("ready_jobs"))
    ready_material = []
    for identifier in ready_ids:
        ...
        ready_material.append({
            ...
            "ready_order": r1._integer(payload.get("ready_order"), "ready order"),
    ...
    material = {
        "tick": r1._tick(event.get("time"), "fingerprint tick"),
...
def _candidate_event(
    ...
    positions = row.get("evidence_event_ids", {}).get(
        "sync_batch_candidate_wait", [],
    )
    if len(positions) != 1:
        return None
    position = positions[0]
    return events[position] if 0 <= position < len(events) else None
...
def audit_asap_sync_document(
    ...
    """Classify every ASAP-SYNC decision into one exhaustive R2 state."""

    normalized, used_exact = _normalize_exact_energy(document)
    events = r1._validate_document(normalized, "gpfp_asap_sync")
    snapshots = _lifecycle_before_events(events)
    base_rows = r1.audit_asap_sync_document(
    ...
    for base in base_rows:
        ...
        legacy_precision_recovered = False
        residual_error = (
            "candidate_wait_residual_energy_after_continuation_reservation_mJ_mismatch"
        )
        candidate = _candidate_event(events, base)
        if (
            not used_exact and residual_error in errors and candidate is not None
            and _legacy_residual_mismatch_is_rounding_only(base, candidate)
        ):
            errors.remove(residual_error)
            legacy_precision_recovered = True

        legacy_block_errors = [
            error for error in errors
            if error.startswith(_LEGACY_BLOCK_ERROR_PREFIXES)
        ]
        semantic_errors = [
            error for error in errors if error not in legacy_block_errors
        ]
        candidate_errors = [
        ...
        except B2R2BatchTraceError:
            if state != B2_STATE_UNCLASSIFIABLE:
                raise
            material = None
        row = dict(base)
        row.update({
            "classified_state_r1": base["classified_state"],
            "classified_state": state,
            "classification_errors": semantic_errors,
            "legacy_block_evidence_errors": legacy_block_errors,
            "legacy_block_is_general_sync_wait": block_present,
            "legacy_block_nonatomic": block_present and state not in ATOMIC_WAIT_STATES,
            "q0_legacy_block": block_present and q == 0,
            "precision_source": (
                "MAX_DIGITS10_EXACT_STRING" if used_exact
                else "LEGACY_DEFAULTFLOAT_PRECISION_6_BOUNDED"
            ),
            "precision_mismatch": bool(candidate_errors),
            "legacy_precision_recovered": legacy_precision_recovered,
...
def audit_asap_sync_trace(path: Path | str, **kwargs: Any) -> list[Dict[str, Any]]:
    return audit_asap_sync_document(r1._load_trace(path), **kwargs)
...
def _decision_positions(
    ...
    for position, event in enumerate(events):
        ...
        if event.get("scheduler") != scheduler:
            continue
        tick = r1._tick(event.get("time"), f"event {position} time")
        if tick in result:
            raise B2R2BatchTraceError("multiple scheduler decisions at one tick")
...
def audit_asap_block_pair_control(
    ...
    normalized, _ = _normalize_exact_energy(block_document)
    events = r1._validate_document(normalized, "gpfp_asap_block")
    snapshots = _lifecycle_before_events(events)
    ...
    for sync in sync_rows:
        ...
        errors: list[str] = []
        ready_ids, ready_energy = r1._jobs(
            decision.get("ready_jobs"), "ready_jobs", errors,
        )
        selected_ids, _ = r1._jobs(
        ...
        following = r1._same_tick_following_events(
        ...
        for event_index, following_event in following:
            if following_event.get("event_type") != "scheduled":
                continue
            identifier = r1._event_job_id(following_event)
...
def audit_asap_block_pair_trace(
    ...
    **kwargs: Any,
    ...
    return audit_asap_block_pair_control(
        sync_rows, r1._load_trace(block_path), **kwargs,
    )
...
def summarize_b2_observations(
    ...
    """Return non-overloaded R2 counters and the atomicity-only metric."""

    state_counts = {state: 0 for state in ALLOWED_STATES}
    for row in rows:
        state = row.get("classified_state")
        if state not in state_counts:
            raise B2R2BatchTraceError(f"unknown R2 state: {state!r}")
    ...
    summary = {
        ...
        "legacy_block_nonatomic_count": sum(
            row.get("legacy_block_nonatomic") is True for row in rows
        ),
        "q0_legacy_block_count": sum(
            row.get("q0_legacy_block") is True for row in rows
        ),
        "precision_mismatch_count": sum(
            row.get("precision_mismatch") is True for row in rows
        ),
        "legacy_precision_recovered_count": sum(
            row.get("legacy_precision_recovered") is True for row in rows
        ),
    ...
    if sum(state_counts.values()) != len(rows):
        raise B2R2BatchTraceError("R2 state partition does not close")
    return summary
