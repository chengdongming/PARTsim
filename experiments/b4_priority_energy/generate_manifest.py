#!/usr/bin/env python3
"""Generate deterministic B4-PE I4A JSON Lines manifests."""

import argparse
from pathlib import Path

from manifest_common import ManifestError, render_manifest


def default_output(phase):
    return Path("/tmp") / f"b4_pe_i4a_manifest_{phase}.jsonl"


def write_manifest(phase, output=None):
    destination = default_output(phase) if output is None else Path(output)
    destination.parent.mkdir(parents=True, exist_ok=True)
    destination.write_bytes(render_manifest(phase))
    return destination
